...
function buildShareText({ eventName, eventDate, eventTime, eventLocation, shareUrl }) {
  return (
    `✅ ${eventName || ''}\n\n` +
...
export default function SocialIcons({ eventName, eventDate, eventTime, eventUrl, eventLocation }) {
  const [copied, setCopied] = useState(false)

  const resolveShareText = () => {
    const shareUrl = eventUrl || window.location.href
    return buildShareText({ eventName, eventDate, eventTime, eventLocation, shareUrl })
  }
  ...
  const handleShare = (event, platform) => {
    ...
    const url = PLATFORMS[platform].buildUrl(shareUrl, shareText)
  ...
  return (
    <Stack direction="row" spacing={0.5}>
      {Object.entries(PLATFORMS).map(([key, platform]) => (
        <Tooltip key={key} title={`Compartilhar no ${platform.label}`}>
          <IconButton
            size="small"
            onClick={(event) => handleShare(event, key)}
            aria-label={`Compartilhar no ${platform.label}`}
            sx={{ '&:hover': { color: platform.color } }}
          >
