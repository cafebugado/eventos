...
// Buscar informações do GitHub pelo username (com retry automático) — não
// depende do Supabase, mas segue o mesmo padrão de withRetry dos demais.
...
export async function getContributorById(supabase, id) {
  return withRetry(
    async () => {
...
export async function createContributor(supabase, contributor) {
  const { data, error } = await supabase
    .from('contribuintes')
    .insert([
      {
...
export async function updateContributor(supabase, id, contributor) {
  const { data, error } = await supabase
    .from('contribuintes')
    .update({
      github_username: contributor.github_username,
...
export async function deleteContributor(supabase, id) {
